  ...
  if (
    ...
    try {
      ...
      if (ctx.request.query && ctx.request.query.token) {
        ...
        if (!token) {
          throw new Error(`Invalid API token: This token doesn't exist`);
        } else {
    ...
    if (
      ...
      return handleErrors(ctx, 'Your account email is not confirmed.', 'account unauthorized');
    }

    if (ctx.state.user.blocked) {
      return handleErrors(
        ctx,
        'Your account has been blocked by an administrator for breaching tos.',
